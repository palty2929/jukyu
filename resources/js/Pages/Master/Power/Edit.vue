<script setup>
import Main from '@/Layouts/Main.vue'
import { Inertia } from '@inertiajs/inertia'
import { Head, Link, useForm } from '@inertiajs/inertia-vue3'
import { useToast } from 'vue-toastification'
const toast = useToast()

const props = defineProps({
    power: Object,
    suppliers: Object,
    bgs: Object
})

const form = useForm({
    start_on: props.power.start_on,
    end_on: props.power.end_on,
    supplier_id: props.power.supplier_id,
    bg_id: props.power.bg_id,
    type: props.power.type,
    power_code: props.power.power_code,
    power_name: props.power.power_name,
    contract_value: props.power.contract_value,
    identity_number: props.power.identity_number
})

const submit = () => {
    form.put(route('power.update', props.power.uuid), {
        onSuccess: () => {
            toast.success('マスタを作成しました')
        }
    })
}

function deleteItem() {
    Inertia.delete(route('power.destroy', props.power.uuid), {
        onBefore: () => confirm('このマスタを完全に削除しますか？'),
        onSuccess: () => {
            toast.success('マスタを削除しました')
        }
    })
}

const types = {
    1: 'FIT電源',
    2: 'FIT送配',
    3: '非調整電源'
}

const getBgs = () => {
    Inertia.get(
        route('power.edit', props.power.uuid),
        { supplier_id: form.supplier_id },
        {
            only: ['bgs'],
            preserveState: true,
            preserveScroll: true,
            replace: true
        }
    )
}
</script>

<template>
    <Head title="エリアマスタの作成" />

    <Main>
        <template #title>エリアマスタの作成</template>

        <template #main>
            <form @submit.prevent="submit">
                <div class="flex border-b mb-8 pb-8">
                    <div class="w-1/2">マスタの有効期限</div>
                    <div class="grow flex flex-col gap-6">
                        <div class="">
                            <div class="m-1 font-bold">開始</div>
                            <input type="date" v-model="form.start_on" class="input input-bordered w-56" required />
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">終了</div>
                            <input type="date" v-model="form.end_on" class="input input-bordered w-56" />
                        </div>
                    </div>
                </div>

                <div class="flex">
                    <div class="w-1/2">マスタ設定</div>
                    <div class="grow flex flex-col gap-6">
                        <div class="">
                            <div class="m-1 font-bold">対象PPS</div>
                            <select @change="getBgs" v-model="form.supplier_id" class="select select-bordered w-full">
                                <option v-for="(supplier, index) in suppliers" :key="index" :value="supplier.value">
                                    {{ supplier.label }}
                                </option>
                            </select>
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">対象BG</div>
                            <select v-model="form.bg_id" class="select select-bordered w-full">
                                <option v-for="(bg, index) in bgs" :key="index" :value="bg.value">
                                    {{ bg.label }}
                                </option>
                            </select>
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">電源種別</div>
                            <select v-model="form.type" class="select select-bordered w-full">
                                <option v-for="(type, key, index) in types" :key="index" :value="key">
                                    {{ type }}
                                </option>
                            </select>
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">系統コード</div>
                            <input
                                type="text"
                                v-model="form.power_code"
                                class="input input-bordered w-full"
                                placeholder="EPM_03_L_dummy"
                            />
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">発電所名</div>
                            <input
                                type="text"
                                v-model="form.power_name"
                                class="input input-bordered w-full"
                                placeholder="EPM_03_L_dummy"
                            />
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">契約電力</div>
                            <input
                                type="text"
                                v-model="form.contract_value"
                                class="input input-bordered w-full"
                                placeholder="EPM_03_L_dummy"
                            />
                        </div>

                        <div class="">
                            <div class="m-1 font-bold">契約識別番号</div>
                            <input
                                type="text"
                                v-model="form.identity_number"
                                class="input input-bordered w-full"
                                placeholder="EPM_03_L_dummy"
                            />
                        </div>
                    </div>
                </div>

                <div class="flex justify-between items-center border-t mt-8 pt-8">
                    <button type="button" @click="deleteItem" class="btn btn-error no-animation">マスタの削除</button>
                    <button type="submit" class="btn btn-primary no-animation" :disabled="!form.isDirty">
                        保存する
                    </button>
                </div>
            </form>
        </template>
    </Main>
</template>
